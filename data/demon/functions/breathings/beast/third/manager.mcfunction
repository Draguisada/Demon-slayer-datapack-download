...
execute rotated ~90 ~-90 positioned ~ ~ ~ run function demon:particles/beast/second/manager
###
execute positioned ^ ^ ^1 run tag @e[tag=!thirdbeast,distance=..2,type=!#demon:nokill] add kill

tellraw @a[tag=debug] "first form beast"

execute as @s run function demon:breathings/manager/cooldowns/fast

execute as @e[tag=kill,type=!#demon:nokill] at @s run damage @s 20 minecraft:player_attack by @a[tag=thirdbeast,limit=1,sort=nearest]
