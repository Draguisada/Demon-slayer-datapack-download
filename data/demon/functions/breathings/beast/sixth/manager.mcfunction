...
###
execute positioned ^ ^ ^1 run tag @e[tag=!sixthbeast,distance=..1.5,type=!#demon:nokill] add 6bf

tellraw @a[tag=debug] "sixth form beast"

execute as @s run function demon:breathings/manager/cooldowns/medium

execute as @e[tag=6bf,type=!#demon:nokill] run damage @s 20 minecraft:player_attack by @a[tag=sixthbeast,limit=1,sort=nearest]


tag @s remove sixthbeast
tag @e[tag=6bf] remove 6bf
